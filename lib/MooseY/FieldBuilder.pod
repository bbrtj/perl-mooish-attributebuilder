...
=head1 SYNOPSIS
	...
	# this property is required on constructor
	has param 'param_name';
	has param 'param_name' => (
		...
	);

	# this property is optional on constructor
	has option 'option_name';
	has option 'option_name' => (
		...
	);

	# this property is not available on constructor
	has field 'field_name';
	has field 'field_name' => (
		...
	);

	# this extends parent property, much like: has '+name_to_extend'
...
This function produces a property that cannot be passed in the constructor:
...
This function produces a property that is required on constructor:
...
This function produces a property that is optional on constructor and has a predicate:
...
=head2 builder / default and required
...
Since the module does not depend on specific OO implementation, the only C<< is => >>
options available are C<ro> and C<rw> (as in base Moose). The author considers
